...
export default function CanjeNew() {
  ...
  const [selectedDeviceId, setSelectedDeviceId] = useState<string | null>('14pro')
  const [device, setDevice] = useState({ model: 'iPhone 14 Pro', storage: '256 GB', battery: 88 })
  const [imei, setImei] = useState('')
  const [powerOn, setPowerOn] = useState(true)
  const [icloudLogout, setIcloudLogout] = useState(false)
  const [deviceColor, setDeviceColor] = useState('space-black')
  const [checks, setChecks] = useState({ screen: true, faceid: true, backglass: false, cameras: true })
  const [selectedNew, setSelectedNew] = useState({ id: '15pro', name: 'iPhone 15 Pro', desc: 'Titanium Blue, 256GB', price: 1099 })
  const [damages, setDamages] = useState<{ id: number; label: string; amount: number }[]>([])

  const availableDevices = [
    { id: '14promax', name: 'iPhone 14 Pro Max', storage: '256GB', image: 'Pro Max', basePrice: 700 },
    { id: '14pro', name: 'iPhone 14 Pro', storage: '256GB', image: 'Pro', basePrice: 650 },
    { id: '13pro', name: 'iPhone 13 Pro', storage: '256GB', image: 'Pro', basePrice: 600 },
    { id: '13', name: 'iPhone 13', storage: '256GB', image: 'Standard', basePrice: 420 },
    { id: '12', name: 'iPhone 12', storage: '256GB', image: 'Standard', basePrice: 300 }
  ]

  const addDamage = (label = 'Scratch', amount = 15) => setDamages(d => [...d, { id: d.length + 1, label: label, amount }])
  const removeDamage = (id: number) => setDamages(d => d.filter(x => x.id !== id))

  const tradeInBase: Record<string, number> = { 'iPhone 14 Pro Max': 700, 'iPhone 14 Pro': 650, 'iPhone 13 Pro': 600, 'iPhone 13': 420, 'iPhone 12': 300 }
  const tradeInCredit = useMemo(() => {
    const base = tradeInBase[device.model] ?? 200
  ...
  const colorOptions = [
    { name: 'Space Black', value: 'space-black', hex: '#3b3c36' },
    { name: 'White', value: 'white', hex: '#f5f5f0' },
    { name: 'Gold', value: 'gold', hex: '#d4a574' },
    { name: 'Silver', value: 'silver', hex: '#a8a9ad' },
    { name: 'Deep Purple', value: 'purple', hex: '#2c445a' }
  ]
                            ...
                            <div className="grid grid-cols-2 gap-6">
                              ...
                              <div className="flex flex-col gap-2">
                                ...
                                <select value={deviceColor} onChange={e => setDeviceColor(e.target.value)} className="rounded-lg border border-input bg-background text-foreground py-3 px-4 focus:outline-none focus:ring-2 focus:ring-ring/20 h-12">
                                  <option value="space-black">Negro Espacial</option>
                                  <option value="white">Blanco</option>
                                  <option value="gold">Dorado</option>
                                  <option value="silver">Plata</option>
                                  <option value="purple">Púrpura Profundo</option>
                                </select>
                              </div>
                            </div>
                            <div className="flex flex-col gap-2">
